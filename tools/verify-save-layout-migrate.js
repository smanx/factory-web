 ...
 *       （热交换器 3×1→2×3、汽轮机 3×3→5×3）会迁移到不与相邻实体重叠的位置，
 *       从而保证读档后每个实体都能被完整选中/拆除。
 *
 * 背景：此前热交换器占地 3×1、汽轮机 3×3；本版改为 2×3、5×3。旧档按旧尺寸摆放，
...
// 迁移后实体占地符合新 def（热交换器 2×3 / 汽轮机 5×3）
const he = ents.filter(e => e.type === 'heat-exchanger');
ok(he.length === 2 && he.every(e => e.w === 2 && e.h === 3), '旧档热交换器迁移后占地为 2×3');
