...
// ===== 铸造厂：太空时代高级熔炼生产建筑（对齐《异星工厂》Space Age，占地 5×5）=====
// 数据（占地/血量/功耗/制造速度/模块槽）全部来自 GAME_DATA（由 factorio-data 生成）。
// 比组装机 III 快得多（crafting_speed=4）、模块槽 4，专用于钨/冶金产品。
class Foundry extends Assembler {
  ...
  powerDemand() { return this.recipe ? POWER_USE['foundry'] : 0; }
}

// ===== 渲染：橙红铸造风格 =====
function drawFoundry(ctx, e, gx, gy, dir, alpha) {
  ...
  ctx.fillStyle = e.crafting ? '#ffe0c0' : '#d8a078';
...
function foundryPanelHtml(e) {
  ...
  h += '<div class="dim">铸造厂：太空时代高级熔炼生产建筑，比组装机 III 更快（官方 crafting_speed 2.0）、模块槽更多（4）。选中后按 R 旋转朝向（流体入口在背部、固体产物经机械臂取走）。</div>';
  h += circuitPanelHtml(e, 'fd');
  return h;
}
function foundryPanelLive(e, api) {
...
// ===== 注册 =====
ENT_CLASSES['foundry'] = Foundry;
DEVICE_RENDER['foundry'] = drawFoundry;
DEVICE_STATUS['foundry'] = e => {
  const s = powerStatusOf(e);
  if (s.consuming) return s.color;
  return e.recipe ? (e.crafting || e.prog > 0 ? 'g' : 'y') : 'r';
};
DEVICE_PANEL['foundry'] = { html: foundryPanelHtml, live: foundryPanelLive, tip: foundryTip, onAction: (a) => circuitPanelAction('fd', a) };
DEVICE_DIR_ROTATE['foundry'] = true;
// 显示详情时，各接口图标所在世界格 + 对应流体名（用于鼠标悬停显示流体名称）
DEVICE_FLUID_ICONS['foundry'] = e => fluidIconFinFout(e, e.x * TILE + TILE * e.w / 2, e.y * TILE + TILE * e.h / 2);
