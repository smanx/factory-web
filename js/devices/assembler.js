...
function assemblerPanelHtml(e) {
  ...
  for (const rid of Object.keys(RECIPES).filter(r => !isChemRecipe(r) && !isCentrifugeRecipe(r) && recipeUnlocked(r))) {
  ...
  const lockedN = Object.keys(RECIPES).filter(r => !isChemRecipe(r) && !isCentrifugeRecipe(r) && !recipeUnlocked(r)).length;
