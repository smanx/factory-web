...
// ===== 电磁工厂：太空时代高级生产建筑（对齐《异星工厂》Space Age，占地 4×4）=====
// 数据（占地/血量/功耗/制造速度/模块槽）全部来自 GAME_DATA（由 factorio-data 生成）。
// 比组装机 III 更快（crafting_speed=2）、模块槽更多（5），专用于电磁产品（超导体等）。
class ElectromagneticPlant extends Assembler {
  constructor(type, x, y) {
    super('electromagnetic-plant', x, y);
  }
  update(dt) {
    this.portFlow();
    ...
    if (this.crafting) {
      ...
      this.prog += dt * asmMult() * (GAME_DATA.deviceStats?.[this.type]?.craftingSpeed ?? 2.0) * this.moduleSpeedMult() * powerFactor(this) * (this.quality ? qualityMult(this.quality) : 1);
      this.spin += dt * 4;
  ...
  // 模块槽位：官方 module_slots=5（GAME_DATA.deviceStats）
  moduleSlotCount() { return GAME_DATA.deviceStats?.[this.type]?.moduleSlots ?? 5; }
  powerDemand() { return this.recipe ? POWER_USE['electromagnetic-plant'] : 0; }
}

// ===== 渲染：蓝紫色电磁风格 =====
function drawElectromagneticPlant(ctx, e, gx, gy, dir, alpha) {
  ...
  ctx.fillStyle = e.crafting ? '#c0dcff' : '#8ab0e8';
...
function electroPanelHtml(e) {
  ...
  h += '<div class="dim">电磁工厂：太空时代高级生产建筑，比组装机 III 更快（官方 crafting_speed 2.0）、模块槽更多（5）。选中后按 R 旋转朝向（流体入口在背部、固体产物经机械臂取走）。</div>';
  h += circuitPanelHtml(e, 'ep');
  return h;
}
function electroPanelLive(e, api) {
...
// ===== 注册 =====
ENT_CLASSES['electromagnetic-plant'] = ElectromagneticPlant;
DEVICE_RENDER['electromagnetic-plant'] = drawElectromagneticPlant;
DEVICE_STATUS['electromagnetic-plant'] = e => {
  const s = powerStatusOf(e);
  if (s.consuming) return s.color;
  return e.recipe ? (e.crafting || e.prog > 0 ? 'g' : 'y') : 'r';
};
DEVICE_PANEL['electromagnetic-plant'] = { html: electroPanelHtml, live: electroPanelLive, tip: electroTip, onAction: (a) => circuitPanelAction('ep', a) };
DEVICE_DIR_ROTATE['electromagnetic-plant'] = true;
// 显示详情时，各接口图标所在世界格 + 对应流体名（用于鼠标悬停显示流体名称）
DEVICE_FLUID_ICONS['electromagnetic-plant'] = e => fluidIconFinFout(e, e.x * TILE + TILE * e.w / 2, e.y * TILE + TILE * e.h / 2);
