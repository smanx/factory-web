...
### 阶段四.16：移除 2.0 已删除物品 + 叠加机械臂（Stack inserter，本迭代新增）

> 已落地说明（本迭代增量）：
> - **移除非官方物品**：彻底移除《异星工厂》2.0 已删除的 `iron-axe` / `steel-axe`（铁斧/钢斧）——
>   从物品、配方、科技门控、近战武器表、初始背包全量清理；存档迁移仍经 `OBSOLETE_ITEMS` 自动清除。
>   至此项目非官方物品仅剩 6 个创造/虚空物品（创造/虚空箱、创造/虚空管道、创造/虚空带）+ 官方卫星 + 火箭本体（发射井内部表示）。
> - **叠加机械臂（Stack inserter）**：接入太空时代 2.0 官方高阶机械臂——
>   `stack-inserter`（堆叠 50 / 血量 160 / 1×1，一次性抓取 4 个物品），由集装箱机械臂升级（官方配方：
>   1 集装箱机械臂 + 1 处理器 + 2 碳纤维 + 10 果冻，0.5s），完整接入机械臂行为（旋转/堆叠抓取/过滤/电路）。
> - **机械臂堆叠数据单源化**：`GAME_DATA.inserterStats.perType` 现从官方原型单源桥接各臂抓取堆叠
>   （bulk-inserter=3、stack-inserter=4、其余=1，stack-inserter 来自官方 `stack_size_bonus`），
>   修复了此前集装箱机械臂堆叠回退为 1 的数据错位。
> - **科技**：新增「叠加机械臂」科技（Space Age，前置电磁学+物流 III+农业，需碳纤维/果冻链），解锁叠加机械臂配方。
> - **校验**：verify-dlc 新增叠加机械臂校验（20 项），全量 18 个校验脚本通过，构建通过。
